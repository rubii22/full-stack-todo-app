...
export default function Alert({ children, type = 'info', className = '' }: AlertProps) {
  ...
  const bgColorClasses = {
    success: 'bg-green-50/30 dark:bg-green-500/20',
    error: 'bg-red-50/30 dark:bg-red-500/20',
    warning: 'bg-yellow-50/30 dark:bg-yellow-500/20',
    info: 'bg-blue-50/30 dark:bg-blue-500/20',
  };

  return (
    <div
      className={`glass-effect border-l-4 p-4 rounded-md ${bgColorClasses[type]} ${typeClasses[type]} ${className}`}
      role="alert"
    >
      <div className="flex">
